...
class Exp_Decode(object):
    ...
    def unit_extract(self, text:list, startl:int, endl:int, bfd:int):
        ...
        current_line = endl
        print("start : {}, end: {}, bfd: {}".format(startl, endl, bfd))
        # 提取info
        ltype = "info"
        ...
        for j in range(bfd+1, endl):
            ...
            print("j: {}, text: {}, score: {}".format(j, text[j], self.score(text[j], ltype)))
    ...
    def descrip_extract(self, lines:list)->list:
        ...
        print("date_pts: ", date_pts, len(date_pts)-1)
    ...
    def info_process(self, exp:dict)->list:
        ...
        for i in exp["info"]:
            ...
            for elem in info_elem:
                if re.sub("[\s]", "", elem) != "":
                    info.append(elem)
        for i in exp["description"]:  # 把description 也加入分析
            info_elem = re.split("\||\s", i)
            for elem in info_elem:
                if re.sub("[\s]", "", elem) != "":
                    info.append(elem)
